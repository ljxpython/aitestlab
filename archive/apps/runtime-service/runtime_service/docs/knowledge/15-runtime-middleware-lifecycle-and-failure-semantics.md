...
每个 Service 必须在 `agent_server.py` 中显式列出实际 Middleware 及其顺序。
...
`RunnableConfig` 后供 Service 的 `agent_server.py` 调用。它只保留执行控制、追踪字段和受控
...
└── services/
    └── <service>/
        ├── agent_server.py      # Middleware 顺序的唯一组合根
