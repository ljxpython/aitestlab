...
1. `langgraph.json -> graphs/<graph_id>.py -> services/<service>/agent_server.py` 是唯一部署链路。
...
Git 历史就是 Legacy 代码存档。旧代码不再复制到仓库级 `archive/`，只有仍有解释价值的历史
文档进入 `docs/archive/`。
...
├── pyproject.toml
├── langgraph.json                      # 只注册 runtime_service.graphs 下的新入口
...
└── src/
    └── runtime_service/
        ...
        └── services/
            ├── __init__.py
            └── reference_agent/       # 首个目标架构验收 Agent
                ├── __init__.py
                ├── agent_server.py    # 唯一组合根，导出 get_agent
                ├── prompts.py
                ├── schemas.py         # 仅 Service 私有 Context/State/Input/Output
                └── README.md
```
...
`agent_server.py/get_agent()` 中直接装配。
...
### 5.1 `agent_server.py` 的组合方式

`agent_server.py` 是 Service 的唯一组合根，直接使用 LangChain/LangGraph/Deep Agents 的官方
...
`create_deep_agent`、显式 `StateGraph`、Interrupt 和 Subagent event 由后续真正需要这些能力的
Service 验证，不塞进首个 reference Agent 制造全家桶 Demo。
...
flowchart LR
    ...
    GRAPH --> SERVER[services/reference_agent/agent_server.py]
...
- 保留旧测试并把失败标记为 skip；
- 把旧代码复制到 `archive/apps/runtime-service/` 继续维护。
