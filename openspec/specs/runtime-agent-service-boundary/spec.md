...
每个新生产或参考 Agent SHALL 位于 `apps/runtime-service/src/runtime_service/services/<agent>/`，并在该模块内维护其 graph、prompt、私有工具和 Agent 默认运行时配置。学习或验收 Demo SHALL 位于 `apps/runtime-service/src/runtime_service/services/demo/<demo>/`，不得作为生产 Service 的实现副本。部署配置 SHALL 只通过 `apps/runtime-service/src/runtime_service/graphs/<graph_id>.py` 的稳定入口暴露生产 Service 或 Demo；共享代码只可用于跨多个 Agent 复用的 middleware、runtime context 或公共工具能力。旧 `apps/runtime-service/runtime_service/` 路径不属于新实现的可导入边界。使用公共 Runtime 的生产 Service MUST 在其 `agent.py` 中显式调用 `resolve_runtime_config` 和 `build_model`，不得由 `graphs/` 层或隐式扫描完成装配。
...
- **THEN** 其实现位于 `src/runtime_service/services/demo/<demo>/`，且 `graphs/<graph_id>.py` 继续作为唯一稳定部署导出，不改变既有 graph ID 或配置入口
