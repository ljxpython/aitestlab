...
MCP tools SHALL 在 Service 组合根构图前加载并过滤；凭据 MUST 只在服务端解析；MCP Tool 名称冲突 MUST 立即失败，不能静默覆盖。

#### Scenario: fake MCP Tool 被加载
- **WHEN** `mcp_demo` 通过 `MultiServerMCPClient` 从本地 stdio fake MCP Server 获取唯一工具
- **THEN** 该工具被显式传给 Agent，且不需要 Platform API
...
- **WHEN** 两个 Thread 使用 `backend_demo`
- **THEN** 每个 Thread 只能读取和写入自己的 Backend 状态

#### Scenario: Skill 按需可见
- **WHEN** `deep_agent_demo` 启动并声明 bundled Skill source
...
每个 R4 Demo SHALL 提供 `async def get_agent(config: RunnableConfig) -> Pregel`、README、独立测试和 `graphs/<graph_id>.py` 导出。introspection 和 fake model 测试 MUST 不连接真实 Sandbox、外部 MCP 或 Platform。

#### Scenario: Demo Graph 可加载
- **WHEN** LangGraph 从 `langgraph.demo.json` 导入任一 R4 graph
