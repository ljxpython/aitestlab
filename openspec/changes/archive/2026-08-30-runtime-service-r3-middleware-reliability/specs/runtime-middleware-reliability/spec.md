...
使用公共 Middleware 的 Agent Service SHALL 在自己的 `agent_server.py` 直接列出 Middleware 顺序，并复用 LangChain 官方调用限制、Tool 错误和 Tool 重试组件。公共 Runtime MUST NOT 自动向所有 Agent 注入全局 Middleware。
