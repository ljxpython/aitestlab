...
每个 Service 只有一个 Top-level Agent 和一个组合根 `agent.py`。顶层
...
```text
1 Service = 1 Top-level Agent = 1 graph_id = 1 agent.py
```
...
Platform API / Assistant Catalog
  ...
  -> runtime_service.services.<service>.agent:get_agent
...
## 1.2 可运行 Demo 是目录规范的一部分

新架构必须提供可复制的参考 Service，不能只留下空的 `services/` 目录。Demo 使用正式的
`agent.py`、`get_agent()`、RuntimeContext、Middleware、测试和 Graph 导出方式；它们
不是临时脚本，也不是生产业务 Agent。

首期固定五个示例：

| 示例 | 覆盖内容 | 阶段 |
| --- | --- | --- |
| `reference_agent` | `create_agent`、显式 Tool、Prompt/Schema、Context、Middleware、Trace | R2 |
| `workflow_demo` | `StateGraph`、Typed State、条件分支、Checkpoint、Interrupt/Resume | R2 |
| `deep_agent_demo` | `create_deep_agent`、Skills、Subagents、StateBackend、子图事件 | R4 |
| `mcp_demo` | Service 私有 MCP、名称冲突、Tool Policy、凭据和副作用边界 | R4 |
| `backend_demo` | Thread Workspace、动态 `get_agent`、Backend 隔离、资源清理 | R4 |

Demo 位于 `src/runtime_service/services/demo/`，对应的 `graphs/<graph_id>.py` 只重导出
入口。生产 `langgraph.json` 默认只注册 `reference_agent`，本地学习使用单独的
`langgraph.demo.json` 注册全部示例。完整目录、README、测试和验收要求见 28 号文档的
“可运行 Demo 计划”章节。
...
│       └── <service_name>/
│           ├── __init__.py
│           ├── agent.py      # 唯一 Top-level Agent 组合根
│           ├── prompts.py
│           ├── tools.py
│           ├── schemas.py
│           ├── README.md
...
| `agent.py` | 必需 | Service 唯一 Top-level Agent 的组合根，导出 `get_agent` |
...
1. 依赖只能从 `agent.py` 指向服务内部模块，内部模块禁止反向导入组合根。
...
```python
from runtime_service.services.test_case.agent import get_agent
...
对于只有一个 Top-level Agent 且组合逻辑仍然短小的 Service，默认把解析、Middleware 列表和
官方 `create_agent`/`create_deep_agent`/`StateGraph` 调用直接写在 `get_agent()` 内，不额外创建
`_build_agent()` 或同类私有 Builder。只有出现真实的多分支资源生命周期、重复装配或需要独立
测试的复杂区块时，才允许增加下划线前缀的 Service 私有辅助函数；该函数仍不能成为公共范式。
...
循环、interrupt 和恢复点。普通 ReAct/tool-calling Agent 直接在 `agent.py` 调用
`create_agent` 或 `create_deep_agent`。

存在 `workflow.py` 时：

- `workflow.py` 负责业务拓扑；
- `agent.py` 负责模型、工具、中间件、Context 和服务导出；
- 两者不能都成为组合根；
- 节点函数不读取进程全局的用户/租户状态。
...
`subagents.py` 导出 `build_subagents(...)`，由 `agent.py` 显式传给
...
内部 Subagent 使用 `agent.py`，不用 `agent.py`，因为它不被 Agent Server 注册。
...
| Top-level Agent | 需要被平台直接调用、独立版本和 graph ID | 提升为新 Service：`agent.py` + 顶层 `graphs/<graph_id>.py` |
...
1. 从对应 Service 的 `agent.py` 导入正式符号；
2. 必要时做稳定别名；
3. 声明 `__all__`。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `11-R0-SVC-001` | Service 位于 `src/runtime_service/services/<name>/`，含组合根和 README | R0 | `src/runtime_service/services/reference_agent/agent.py:54`；`reference_agent/README.md`；`workflow_services/demo/agent.py`；`workflow_services/demo/README.md` | `tests/test_r0_baseline.py:test_service_entrypoints_return_pregel`；`test_r0_services_have_readme_and_dedicated_tests` | `uv run --frozen pytest tests/test_r0_baseline.py -q`：`14 passed` | `implemented-local` | ✅ | R0 两个 Service 已闭合 |
...
| `11-R0-SVC-003` | 每个 Service 暴露异步 `get_agent(config) -> Pregel` | R0 | `services/reference_agent/agent.py:54`；`services/demo/workflow_services/demo/agent.py:11` | `tests/test_r0_baseline.py:52-61` | 同上；两个入口返回 `Pregel` | `implemented-local` | ✅ | 不证明真实 Agent Server Durable |
| `11-R0-SVC-004` | 生产注册只暴露 Top-level `reference_agent` | R0 | `langgraph.json:5-10`；生成的 `deploy/Dockerfile:20` | `tests/test_r0_baseline.py:test_production_config_registers_only_reference_agent`；`test_docker_graph_registry_matches_production_config` | R0 基线 `14 passed`，生产配置与容器注册表一致 | `implemented-local` | ✅ | 修改生产配置后必须重新生成 Dockerfile |
| `11-R0-SVC-005` | `workflow_demo` 作为确定性 StateGraph 示例 | R0 | `services/demo/workflow_services/demo/workflow.py:8-15` | `tests/test_r0_baseline.py:77-81` | 确定性输入 `message=hello` 返回预期响应 | `implemented-local` | ✅ | 条件分支、Checkpoint、Interrupt/Resume 属于 R2，已在 R2 对齐目录单独验收；R0 只证明最小确定性执行 |
| `11-R0-SVC-006` | 注册前应有 Service README、入口测试和无外部副作用的 fake 验证 | R0 | `services/reference_agent/README.md`；`services/demo/workflow_services/demo/README.md`；两个 `agent.py` | `tests/test_r0_baseline.py:test_r0_services_have_readme_and_dedicated_tests`；`test_service_entrypoints_return_pregel`；`test_reference_agent_uses_deterministic_fake_model`；`test_workflow_demo_has_deterministic_state_transition` | R0 基线：`14 passed`；两个 R0 Service 的 README、专属测试和无外部副作用验证均存在 | `implemented-local` | ✅ | 真实模型 E2E 仍按显式开关单独记录 |
| `11-R0-SVC-007` | Service 之间不互相导入，组合根依赖服务私有模块和公共 Runtime | R0 | `src/runtime_service/services/*` 导入链 | `tests/test_r0_baseline.py:test_services_do_not_import_each_other` | AST 依赖方向门禁通过；当前 Service 之间无私有实现导入 | `implemented-local` | ✅ | 新增 Service 时由该门禁自动约束 |
...
| `11-R2-REF-002` | `create_agent(..., context_schema=RuntimeContext)` 接收 invoke Context，业务 Context 不写入 configurable/Prompt | `services/reference_agent/agent.py:110-121`；`runtime/resolver.py` | `tests/services/reference_agent/test_agent.py:test_context_override_is_resolved_before_model_creation`；R1 Auth/Context tests；GraphHarbor Durable Context failure test | 本地 Context 合同与 GraphHarbor Durable `temperature=0` / 未知字段链路均有证据 | `implemented-chain` | ✅ | 继续由 R1/R5 矩阵维护跨边界契约 |
| `11-R2-REF-003` | 当前 R2 合同要求缺少 Auth 时 fail-closed；本地测试必须显式提供 test-only adapter | `services/reference_agent/agent.py:57-75`；`agent.py:77-87` | `tests/services/reference_agent/test_agent.py:test_agent_requires_authenticated_server_facts`；显式 fake model 测试 | 空配置返回 `runtime.auth.missing_principal`；显式 fake model + Auth fixture 通过；归档 R2 的空配置成功场景由 active spec supersede | `implemented-local` | ✅ | 归档 R2 历史不修改；后续以 active spec 为准 |
| `11-R2-WF-001` | `workflow_demo` 使用 Typed StateGraph，拥有确定节点、边和至少一个条件分支 | `services/demo/workflow_services/demo/schemas.py`；`workflow.py` | `tests/services/workflow_services/demo/test_agent.py:test_workflow_demo_routes_to_one_conditional_branch` | 两条分支测试通过；默认 R0 响应也保持通过 | `implemented-local` | ✅ | R2 本地证据，不代表生产 Durable |
| `11-R2-WF-002` | `workflow_demo` 提供人工确认或可恢复 Interrupt/Resume，并证明恢复不重复已完成步骤 | `services/demo/workflow_services/demo/workflow.py:confirm`；`agent.py:_AGENT` | `tests/services/workflow_services/demo/test_agent.py:test_workflow_demo_interrupt_resume_does_not_repeat_completed_step`；`test_workflow_demo_rejects_invalid_resume_without_completing_run` | `InMemorySaver` 同 Thread 暂停/恢复通过；`prepared_count == 1`；非法 Resume 返回稳定错误 interrupt | `implemented-local` | ✅ | R6 仍需验证 PostgreSQL/Redis、Worker 重启和跨进程恢复 |
| `11-R2-LIFE-001` | 无 Thread 资源的静态 Graph 重复调用拓扑一致；只有 Thread/Run 资源才动态构图 | `workflow_services/demo/agent.py:9-19`；`reference_agent/agent.py:77-151` | `tests/services/workflow_services/demo/test_agent.py:test_workflow_demo_static_topology_is_stable`；`tests/services/reference_agent/test_agent.py:test_reference_agent_topology_is_stable_across_runtime_bindings` | R2/R0 相关测试：`38 passed`；workflow 重复入口返回同一实例，reference 不同模型绑定返回不同实例但 nodes、edges、输入/输出 schema 结构一致 | `partial` | ❌ | 测试已证明拓扑稳定，但 reference 仍按每次运行配置动态重建；需收敛为静态缓存，或记录并批准“运行时模型绑定导致动态构图”的 factory 例外 |
| `11-R2-DOC-001` | 两个 R2 Service 均有独立 README、标准 demo 配置和最小 Service 测试 | `services/reference_agent/README.md`；`services/demo/workflow_services/demo/README.md`；`langgraph.demo.json` | `tests/test_r0_baseline.py`；`tests/services/workflow_services/demo/test_agent.py` | README、demo config、workflow 专属测试均存在并通过 | `implemented-local` | ✅ | R6 运行手册仍单独维护 |
| `11-R2-CHAIN-001` | 两个 R2 Graph 均可被标准 Agent Server 配置加载、查询并执行，且不触发外部副作用 | `langgraph.demo.json`；`graphs/*.py`；两个 Service `agent.py` | `tests/integration/test_agent_server_auth.py`；`tests/integration/test_workflow_demo_agent_server.py` | reference GraphHarbor Durable `3 passed`；workflow 需要注册 demo graph 的 `RUNTIME_DURABLE_DEMO_URL`，本轮未执行 | `not-executed` | ❌ | 仅 reference 的 Durable chain 已有当前证据；提供 demo Durable 环境后再闭合两个 Graph 的整体要求 |
| `11-R2-SCOPE-001` | R2 目标设计与 active spec 对同一能力范围保持一致，并为归档冲突保留迁移记录 | `11-agent-service-directory-architecture.md:130`；`openspec/specs/runtime-agent-service-integration/spec.md`；归档 R2 文件 | `openspec validate runtime-agent-service-integration --strict --no-interactive` | 命令返回 `Specification 'runtime-agent-service-integration' is valid`；active spec 已 sync；归档文件保留历史且由当前 delta 记录迁移原因 | `implemented-local` | ✅ | 归档 R2 历史不修改；后续以 active spec 为准 |
...
`create_agent`/Context/Resolver/真实模型链、workflow 条件分支、Interrupt/Resume 和专属测试
已成立；reference 的 GraphHarbor Durable chain 已通过，但 workflow 的 Durable chain 本轮未执行，
且 reference 的静态/动态生命周期仍不完全符合 11 号文档。因此 R2 仍不能标记为无条件完成。
