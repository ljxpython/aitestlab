...
默认测试必须可在本地、无真实模型、无 PostgreSQL、无 Redis、无外部 Sandbox 的条件下运行。
真实基础设施测试单独执行，不阻塞普通提交反馈。
...
- E2E 只模拟模型和外部 SaaS，尽量真实运行 Agent、Middleware、Tool 和服务入口；
...
遵循 Open SWE 的边界替换方式：模型、外部 SaaS 和凭证服务可以 fake，Agent Service、
Middleware、工具、LangGraph 入口和本地隔离 Backend 尽量真实。首期不建设 Playwright、
Electron 或业务 SaaS 的完整复制品。
